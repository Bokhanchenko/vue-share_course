  ...
  <v-container class="text-xs-center">
    ...
    <v-container class="mt-3" v-else>
      ...
      <v-layout row wrap>
        <v-flex xs12 sm6 v-for="post in userPosts" :key="post._id">
          <v-card class="mt-3 ml-1 mr-2" hover>
            <v-btn class="info" floating fab small dark @click="loadPost(post)">
              <v-icon>edit</v-icon>
            </v-btn>
            <v-btn class="error" floating fab small dark @click="handleDeleteUserPost(post)">
    ...
    <v-dialog xs12 sm6 offset-sm3 persistent v-model="editPostDialog">
      <v-card>
        ...
        <v-container>
          <v-form v-model="isFormValid" lazy-validation ref="editForm" @submit.prevent="handleUpdateUserPost">
...
export default {
  ...
  methods: {
    ...
    handleUpdateUserPost() {
      // Update User Post
      const { postId, title, imageUrl, categories, description, user} = this;
    ...
    handleDeleteUserPost(post) {
      ...
      this.$store.dispatch('deleteUserPost', { postId: post._id })
    },
