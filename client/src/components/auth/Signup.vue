    ...
    <v-layout row wrap>
      <v-flex xs12 sm6 offset-sm3>
        <v-card color="success" dark>
          <v-container>
            <v-form v-model="isFormValid" lazy-validation ref="singupForm" @submit.prevent="handleSignupUser">
              <v-layout row>
                <v-flex xs12>
                  <v-text-field
                    :rules='usernameRules'
              ...
              <v-layout row mt-4>
                <v-flex xs12>
                  ...
                  <h3>Already have an account?
                    <router-link to="/signin" style="color:#ffeb3b">Sugnin</router-link>
                  </h3>
                </v-flex>
              </v-layout>
            </v-form>
  ...
  data() {
    return {
      ...
      usernameRules: [
        ...
        username => username.length < 10 || "Username must be valid"
      ],
      emailRules: [
        email => !!email || 'Email is required',
        email => /.@+./.test(email) || "Email must be less then 4 characters"
      ],
      passwordRules: [
