<template>
  <v-container v-if="getPost" class="mt-3" flex center>
    <!--Post Card-->
          ...
          <v-layout row>
            <v-flex xs12>
              <v-text-field
                ...
                v-model="messageBody"
                clearable
                :append-outer-icon="messageBody && 'sand'"
                label="Add Message"
                type="text"
                prepend-icon="email"
                required
  ...
  methods: {
    handleAddPostMessage() {
      ...
      this.$apollo.mutate({
        ...
        .catch((err) => {
          console.error('Error in handleAddPostMessage', err)
        })
    },
    ...
    checkIfPostLiked(postId) {
      // check if user favorites includes post with id or 'postId'
      this.postLiked = this.userFavorites && this.userFavorites.some(fave => fave._id === postId);
    ...
    handleLikePost() {
      this.$apollo.mutate({
        ...
        .catch(err => console.error('Error in handleLikePost', err))
