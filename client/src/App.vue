  ...
  <v-app style="background: #E3E3EE">
    ...
    <main>
      <v-container class="mt-4">
        ...
        <!--Auth Snackbar-->
        <v-snackbar v-model="authSnackbar" class="success" :timeout="5000" bottom left>
          <v-icon class="mr-3">check_circle</v-icon>
          <h3>You are now signed in!</h3>
          <v-btn darck flat @click="authSnackbar = false">Close</v-btn>
        </v-snackbar>

        <!--Auth Error Snackbar-->
        <v-snackbar v-if="authError" v-model="authSnackbar" class="info" :timeout="5000" bottom left>
          <v-icon class="mr-3">cancel</v-icon>
          <h3>{{ authError.message }}}</h3>
          <v-btn darck flat to="/signin ">Signin</v-btn>
        </v-snackbar>
...
export default {
  ...
  watch: {
    user(newVal, oldVal) {
      ...
      if (oldVal === null) this.authSnackbar = true
    },
    authError(value) {
