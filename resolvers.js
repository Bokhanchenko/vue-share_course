  ...
  Query: {
    ...
    infiniteScrollPosts: async (_, { pageNum, pageSize }, { Post }) => {
      ...
      if (pageNum === 1) {
        ...
        // if pageNum is greater then one, figure out gow many documents to skip
      ...
      const totalDecs = await Post.countDocuments();
      const hasMore = totalDecs > pageSize * pageNum;
  ...
  Mutation: {
    ...
    updateUserPost: async (_, { postId, userId, title, imageUrl, categories, description }, { Post }) => {
      const post = await Post.findOneAndUpdate(
        ...
        { $set: { postId, userId, title, imageUrl, categories, description }},
        { new: true }
      );
    ...
    likePost: async (_, { postId, username }, { Post, User }) => {
      ...
      // Find User, add id of post to its favorites array (whitch will be populate as Post)
    ...
    unlikePost: async (_, { postId, username }, { Post, User }) => {
      // Find Post? remove 1 from its 'like' value
      const post = await Post.findOneAndUpdate(
        { _id: postId },
        { $inc: { likes: -1 }},
        { new: true }
      );
      // Find User, remove id of post from its favorites array (witch will be populate as Post)
      const user = await User.findOneAndUpdate(
        { username },
